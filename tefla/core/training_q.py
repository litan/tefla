...
class SupervisedTrainerQ(object):
    def __init__(self, model, cnf, input_shape, trainable_scopes=None, training_iterator=BatchIterator(32, False),
    ...
    def fit(self, data_set, weights_from=None, weights_exclude_scopes=None, start_epoch=1,
        ...
        self.num_batchs_in_epoch = int(math.ceil(len(data_set.training_X) / self.training_iterator.batch_size))
        self.num_val_batchs_in_epoch = int(math.ceil(len(data_set.validation_X) / self.validation_iterator.batch_size))

        self._setup_input_queue()
    ...
    def _train_loop(self, data_set, weights_from, weights_exclude_scopes, start_epoch, resume_lr,
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            ...
            batch_iters_per_epoch = self.num_batchs_in_epoch
                ...
                for epoch in moves.xrange(start_epoch, self.num_epochs + 1):
                    ...
                    for batch_num in range(1, batch_iters_per_epoch + 1):
                        ...
                        if (epoch - 1) % summary_every == 0 and batch_num < 2:
                            ...
                            batch_train_sizes.append(len(training_predictions_e))
                            ...
                            training_loss_e, num_batch_elems_e, _ = sess.run(
                                [self.regularized_training_loss, self.num_batch_elems, self.optimizer_step],
                                feed_dict=feed_dict_train)
                            batch_train_sizes.append(num_batch_elems_e)
                    ...
                    for batch_num in range(1, self.num_val_batchs_in_epoch + 1):
                        ...
                        if (epoch - 1) % summary_every == 0 and batch_num < 2:
                            ...
                            validation_predictions_e, validation_loss_e, validation_Xb, validation_yb = sess.run(
                                [self.validation_predictions, self.validation_loss, self.inputs, self.target])
    ...
    def _setup_summaries(self):
        ...
        with tf.name_scope('summaries'):
            ...
            for key, val in six.iteritems(self.training_end_points):
            ...
            for key, val in six.iteritems(self.validation_end_points):
