...
class SupervisedTrainerN(object):
    ...
    def _train_loop(self, data_set, weights_from, start_epoch, resume_lr, summary_every,
        ...
        with tf.Graph().as_default(), tf.device('/cpu:0'):
            ...
            inputs = tf.placeholder(tf.float32, shape=(None, 448, 448, 3), name="input")
            target = tf.placeholder(tf.int64, shape=(None,), name="target")
            ...
            with tf.Session(
                ...
                for epoch in xrange(start_epoch, self.num_epochs + 1):
                    ...
                    for batch_num, (Xb, yb) in enumerate(self.training_iterator(training_X, training_y), start=1):
                        feed_dict_train = {inputs: Xb, target: self._adjust_ground_truth(yb),
                    ...
                    for batch_num, (validation_Xb, validation_yb) in enumerate(
                        ...
                        feed_dict_validation = {inputs: validation_Xb,
                                                target: self._adjust_ground_truth(validation_yb)}
